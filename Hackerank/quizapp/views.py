...
from quizapp.forms import RegistrationForm,LoginForm
...
class HomeSubView(TemplateView):
...
class QuestListView(View):
    ...
    def post(self,request,*args,**kwargs):
        ...
        print(total_mark,question_attended,wrong_answer_count,right_answer_count)
        category=kwargs.get("cat")
        mode=kwargs.get("mode")
        result=''
        total=Questions.objects.filter(mode=mode,category__name=category).aggregate(Sum("mark")).get('mark__sum')
        if total/2 <= total_mark:
            result='passed the exam'
        else:
            result='failed the exam'
        # total=total.get('mark___sum')
        print(total)
        data=QuizRecord.objects.create(total_mark=total_mark,right_answer_count=right_answer_count,wrong_answer_count=wrong_answer_count,user=request.user)
        return render(request,"quiz-mark.html",{'total_mark':total_mark,"question_attended":question_attended,'result':result})
